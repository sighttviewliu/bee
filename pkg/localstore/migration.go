...
func migrateYuj(db *DB) error {
	...
	if c > 0 {
		return errors.New("pinned content detected. bee v0.6.0 cannot migrate a store with pinned content. please refer to the release notes for more information.")
	}
	...
	truncate := func(i shed.Index) error {
		...
		err := i.Iterate(func(item shed.Item) (stop bool, err error) {
			...
			count++
			if count%lim == 0 {
				db.logger.Infof("truncate writing batch. processed %d", count)
	...
	start := time.Now()
	db.logger.Info("truncating indexes")
	...
	for _, v := range []struct {
		...
		db.logger.Infof("truncating %s index", v.name)
		err := truncate(v.idx)
		if err != nil {
			return fmt.Errorf("truncate %s index: %w", v.name, err)
		}
		db.logger.Infof("truncated %d %s index entries", v.name, count)
	}
	err = db.gcSize.Put(0)
	if err != nil {
	...
	db.logger.Infof("done truncating indexes. took %s", time.Since(start))
	return nil
}
