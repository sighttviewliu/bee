...
func (k *Kad) connect(ctx context.Context, peer swarm.Address, ma ma.Multiaddr) error {
	...
	if err != nil {
		...
		if k.quickPrune(peer, failedAttempts) || failedAttempts > maxConnAttempts {
...
func (k *Kad) quickPrune(addr swarm.Address, attempts int) bool {
	...
	if snapshot == nil || snapshot.LastSeenTimestamp == 0 {
		return true
	}
